...
def get_content(module_links):
    content = {}
    os.system("clear")
    for key, value_list in tqdm(module_links.items(), desc="Getting contents for each module"):
        content[key] = []
...
def to_markdown(module_names):
    ...
    with tqdm(total=len(module_names.items()), desc="\rProcessing data for each module:") as outer_bar:
        ...
        for key, value_list in module_names.items():
            ...
            with tqdm(total=len(value_list), desc=f"\r{key}") as inner_bar:
...
def write_to_files(module_names):
    ...
    with tqdm(total=len(module_names.items()), desc="\rWriting files to system:") as outer_bar:
        ...
        for key, value_list in module_names.items():
            ...
            with tqdm(total=len(value_list), desc=f"\rLoading files to module {key} folder:") as inner_bar:
                i = 0
                ...
                for value in value_list:
                    ...
                    if start_index is not None:
                        ...
                        with open(filename, 'w') as f:
                            f.write('\n'.join(lines[3:]))
                    inner_bar.update()  # Update the inner progress bar
